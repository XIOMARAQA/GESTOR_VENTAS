...
async function load() {
  ...
  err.value = ''
  try {
    const { data } = await api.get<{ results?: Row[] }>('/inventario/marcas/?page_size=500&ordering=nombre')
    rows.value = Array.isArray(data) ? data : (data.results ?? [])
  } catch (e) {
    err.value = listLoadErrorMessage(e, 'el catálogo de marcas')
    rows.value = []
  } finally {
    loading.value = false
  }
...
async function guardar() {
  ...
  try {
    ...
    if (editingId.value == null) {
      await api.post('/inventario/marcas/', body)
    } else {
      await api.patch(`/inventario/marcas/${editingId.value}/`, body)
    }
...
async function inactivar(r: Row) {
  ...
  const ok = window.confirm(
    `¿Inactivar la marca «${r.nombre}»? No se elimina de la base de datos; puede reactivarla desde Editar.`,
  )
  if (!ok) return
  rowBusy.value = r.id
  err.value = ''
  try {
    await api.patch(`/inventario/marcas/${r.id}/`, { activo: false })
    await load()
  } catch (e) {
    if (axios.isAxiosError(e) && e.response?.data) {
      err.value = drfMsg(e.response.data)
    } else {
      err.value = 'No se pudo inactivar la marca.'
    }
  } finally {
    rowBusy.value = null
  }
  ...
  <div class="page">
    <header class="head">
      <div>
        ...
        <p class="lead">
          Marcas comerciales de su catálogo. Son <strong>opcionales</strong> en cada producto: puede asignarlas desde la
          ficha del ítem cuando lo necesite.
        </p>
      </div>
        ...
        <table class="t">
          ...
          <tbody>
            <tr v-for="r in filtradas" :key="r.id">
              ...
              <td class="td-act">
                <div class="act-icons" role="group" :aria-label="`Acciones para marca ${r.nombre}`">
                  <button
                    type="button"
                    class="icon-act icon-act--edit"
                    title="Editar"
                    :aria-label="`Editar marca ${r.nombre}`"
                    @click="openEdit(r)"
                  >
                  ...
                  <button
                    ...
                    :aria-label="`Inactivar marca ${r.nombre}`"
    ...
    <Teleport to="body">
      <div v-if="showModal" class="backdrop" @click.self="closeModal">
        <div class="modal" role="dialog" aria-modal="true">
          <h2 class="mt">{{ editingId == null ? 'Nueva marca' : 'Editar marca' }}</h2>
