...
async function fetchRows() {
  const { data } = await api.get<{ results?: Row[] }>(
    '/core/clientes/?page_size=500&ordering=razon_social',
  )
...
async function load() {
  ...
  try {
    ...
    err.value = listLoadErrorMessage(e, 'el listado de clientes')
    rows.value = []
  } finally {
    loading.value = false
  }
...
async function descargarPlantilla() {
  err.value = ''
  try {
    const { data } = await api.get('/core/clientes/plantilla-excel/', { responseType: 'blob' })
...
async function onImportFile(ev: Event) {
  ...
  try {
    ...
    }>('/core/clientes/importar-excel/', fd)
...
async function openEdit(r: Row) {
  ...
  formErr.value = ''
  try {
    const { data } = await api.get<Row>(`/core/clientes/${r.id}/`)
    form.value = {
    ...
    err.value = 'No se pudo cargar el cliente para editar.'
  }
}
...
async function guardar() {
  ...
  if (!doc) {
    formErr.value = 'El documento (DNI/RUC) es obligatorio.'
    return
  }
  if (!f.razon_social.trim()) {
    formErr.value = 'La razón social o nombre completo es obligatorio.'
    return
  }
  saving.value = true
  try {
    const body: Record<string, unknown> = {
      documento: doc.slice(0, 20),
    ...
    if (editingId.value == null) {
      await api.post('/core/clientes/', body)
    } else {
      await api.patch(`/core/clientes/${editingId.value}/`, body)
    }
...
async function inactivar(r: Row) {
  ...
  err.value = ''
  try {
    await api.patch(`/core/clientes/${r.id}/`, { activo: false })
    await load()
  } catch (e) {
    if (axios.isAxiosError(e) && e.response?.data) {
      err.value = drfMsg(e.response.data)
    } else {
      err.value = 'No se pudo inactivar el cliente.'
    }
  } finally {
    rowBusy.value = null
  }
  ...
  <div class="page">
    <header class="head">
      <div>
        ...
        <p class="lead">
          Personas o empresas a las que factura: documento, razón social o nombre, datos de contacto y dirección. En el
          formulario puede usar <strong>Consultar SUNAT</strong> (RUC de 11 dígitos) o
          <strong>Consultar RENIEC</strong> (DNI de 8 dígitos) para sugerir el nombre.
        </p>
      </div>
    ...
    <div class="filters">
      ...
      <label class="f">
        <span class="flab">Razón social / nombre</span>
        ...
        <table class="t">
          ...
          <tbody>
            <tr v-for="r in filtradas" :key="r.id">
              ...
              <td class="td-act">
                <div class="act-icons" role="group" :aria-label="`Acciones cliente ${r.id}`">
                  <button
                    type="button"
    ...
    <Teleport to="body">
      <div v-if="showModal" class="backdrop" @click.self="closeModal">
        <div class="modal" role="dialog" aria-modal="true">
          <h2 class="mt">{{ editingId == null ? 'Nuevo cliente' : 'Editar cliente' }}</h2>
          <p v-if="formErr" class="form-err">{{ formErr }}</p>

          <div class="doc-row">
            <label class="field field--grow">
              <span class="lab">N° documento (DNI o RUC)</span>
            ...
            <div class="consult-btns">
              <button
                ...
                title="Autocompletar con SUNAT (11 dígitos)"
              ...
              <button
                ...
                title="Autocompletar con RENIEC / Decolecta (8 dígitos)"
          ...
          <p class="hint">
            RENIEC y SUNAT solo sugieren el nombre; puede corregirlo antes de guardar. Requiere
            <code>APIS_NET_PE_TOKEN</code> (Decolecta) en el servidor.
          </p>

          <label class="field">
            <span class="lab">Razón social o nombre completo</span>
