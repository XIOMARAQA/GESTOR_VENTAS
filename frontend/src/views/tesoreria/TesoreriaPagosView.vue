...
const pagosEndpoint = computed(() => {
  ...
  return `/tesoreria/pagos-recibidos/?${params}`
})
...
async function loadPagos() {
  ...
  error.value = ''
  try {
    const { data } = await api.get<{ results?: PagoRow[] }>(pagosEndpoint.value)
    rows.value = Array.isArray(data) ? data : (data.results ?? [])
  } catch (e) {
    error.value = listLoadErrorMessage(e, 'los pagos recibidos')
    rows.value = []
  } finally {
    loading.value = false
  }
...
watch(pagosEndpoint, () => loadPagos(), { immediate: true })
...
function clearRouteDocumentos() {
  void router.replace({ path: '/tesoreria/pagos', query: {} })
}
...
const emptyMsg =
  'Aún no hay pagos registrados. Al emitir una venta al contado se genera automáticamente un pago asociado a la cobranza. Las ventas a crédito solo aparecen aquí cuando registre cobros sobre la cobranza correspondiente.'

const filteredEmptyMsg =
  'No hay pagos que coincidan con los filtros. Pruebe ampliar fechas o limpiar criterios.'
...
async function revertirPago(row: PagoRow) {
  if (row.cobranza == null) return
  const ok = window.confirm(
    '¿Revertir este pago? Se descontará el monto de la cobranza asociada y el movimiento dejará de figurar aquí. Use esta opción si registró un cobro por error.',
  )
  if (!ok) return
  revertingId.value = row.id
  try {
    await api.post(`/tesoreria/pagos-recibidos/${row.id}/revertir/`)
    await loadPagos()
  } catch (e) {
  ...
  <div class="pagos-page">
    <header class="page-head">
      <div class="page-head__top">
        <div>
          ...
          <p class="page-lead">
            Movimientos de cobro vinculados a comprobantes. Filtre como en comprobantes; si se equivocó al cobrar, use
            <strong>Revertir</strong> en la fila (actualiza la cuenta por cobrar).
          </p>
        </div>
      </div>
      ...
      <div class="panel-inline">
        <span class="panel-inline__text">
          Registrar abonos:
          <RouterLink class="text-link" to="/tesoreria/cobranzas">Cuentas por cobrar</RouterLink>
          ·
          <RouterLink class="text-link" to="/ventas/documentos">Comprobantes</RouterLink>
        </span>
      </div>
    </header>

    <p v-if="documentosDesdeRuta.length" class="route-banner">
      Vista acotada por enlace desde cuentas por cobrar ({{ documentosDesdeRuta.length }} comprobante(s)).
    ...
    <section class="filters" aria-label="Filtros de pagos">
      <div class="filter-row filter-row--primary">
        ...
        <label class="filter-field filter-field--date">
          <span class="filter-label">Emisión hasta</span>
      ...
      <table class="table">
        ...
        <tbody v-else>
          <tr v-for="row in rows" :key="row.id">
            <td class="td-tipo" :title="row.tipo_documento != null ? String(row.tipo_documento) : undefined">
              {{ labelTipoCorto(row.tipo_documento) }}
            </td>
            <td>
              <RouterLink
                v-if="row.documento_venta_id != null"
                class="cell-link"
                :to="{ path: '/ventas/documentos', query: { documento: String(row.documento_venta_id) } }"
              >
                {{ row.documento_serie_numero ?? `#${row.documento_venta_id}` }}
              </RouterLink>
              <span v-else class="muted-cell">Sin comprobante</span>
            </td>
            ...
            <td class="td-cob">
              <span v-if="row.cobranza != null" class="mono">{{ row.cobranza }}</span>
            ...
            <td class="td-actions">
              <button
                ...
                :title="
                  canRevertir(row)
                    ? 'Quita este pago y descuenta el monto de la cobranza'
                    : 'Sin cobranza vinculada; no se puede revertir aquí'
                "
